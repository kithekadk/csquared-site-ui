...
export class CountryService {
  ...
  countrypageImages:string[]=[];
  countries:string[]=[];
  ugandaTitle : string = '';
  getCountryInfo(){
    return this.http.get(`${this.baseUrl}api/v1/slides`)
  }
  ...
  private countrySubject = new BehaviorSubject<string>('togo');
  public setCountry$ = this.countrySubject.asObservable();

  
  getCountry(id:string){
    return this.http.get(`${this.baseUrl}api/v1/countries/${id}`).pipe(map(res => {
      return res;
    }))
  }

  createOrder(neworder:order){
    return this.http.post<order>(`${this.baseUrl}api/v1/order`, neworder);
  }

  contactCsquared(issue:contactus){
    return this.http.post<contactus>(`${this.baseUrl}api/v1/contact`,issue);
  }

  getOneCountry(code: string): Countries {

    let country = this.countriesheader.find(c => {
