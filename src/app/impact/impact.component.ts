...
export class ImpactComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.apiservice.getImpacts().subscribe(res=>{
      ...
      for(let cat of res['data']){
        let slug = cat.slug
        let categoryid = cat.impact_type['id']
        this.impactSlug.push(slug)
        console.log(cat.impact_type['name']);

        // Impact IDS
        this.impactCategory.push(categoryid)
        this.impactCategoryID = [... new Set(this.impactCategory)]

        // Impact names
        this.impactnames.push(cat.impact_type['name'])
        this.impactCategoryName = [...new Set(this.impactnames)]
  ...
  filterCategory(categoryid:string) {
    this.mergedArray = this.mergedArray2.filter(el=>categoryid===''?true:el.category === categoryid || el.category==='')
